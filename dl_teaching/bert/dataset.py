...
# Modified from https://github.com/coaxsoft/pytorch_bert/blob/master/bert/dataset.py
...
class IMDBBertDataset(Dataset):
    ...
    def __init__(self, path, ds_from=None, ds_to=None, should_include_text=False):
        ...
        if should_include_text:
            self.columns = ['masked_sentence', self.MASKED_INDICES_COLUMN, self.SEGMENT, 'sentence', self.TARGET_COLUMN,
                            self.TOKEN_MASK_COLUMN,
                            self.NSP_TARGET_COLUMN]
        else:
            self.columns = [self.MASKED_INDICES_COLUMN, self.SEGMENT, self.TARGET_COLUMN, self.TOKEN_MASK_COLUMN,
                            self.NSP_TARGET_COLUMN]
    ...
    def __getitem__(self, idx):
        ...
        seg = torch.Tensor(item[self.SEGMENT]).long()
    ...
    def _create_item(self, first: typing.List[str], second: typing.List[str], target: int = 1):
        ...
        seg_sentence = [0]*(len(updated_first) + 1) + [1]*len(updated_second)
    ...
    if __name__ == "__main__":
        ...
        ds = IMDBBertDataset('IMDB Dataset.csv', ds_from=0, ds_to=5,
